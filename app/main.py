...
async def get_scores(
    ...
    """
    Get CFB game scores. Returns raw data from CFBD API for debugging.
    """
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            # Return raw data for now to debug field names
            return {
                "year": year,
                "week": week,
                "team": team,
                "total_games": len(games),
                "raw_games": games[:2] if games else []  # Just first 2 games for debugging
            }
